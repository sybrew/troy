...
- When outside global namespace:
	...
	7. Import only first-order names (one identifier after the namespace prefix). Do not import nested paths. Qualify the rest at the call site: `use Troy\Server\{ Admin, API };` then `Admin\Notice\Persistent`, not `use Troy\Server\{ Admin\Notice\Persistent, API };`.
