---
description: "Use when editing PHP, WordPress PHP, SQL-in-PHP, or mixed PHP and HTML template files in this repository. Covers Troy PHP style, namespace rules, SQL formatting, and template formatting."
applyTo: "**/*.php"
---
...
- Add trailing commas on multiline array items and function calls. Do not add them on function, method, closure, or `fn()` parameter lists, closure `use (` lists, or control-structure conditions in PHP 7.4 packages (PHP 8.0 syntax). `src/troy-server/**` (PHP 8.4+) may use trailing commas on multiline parameter lists.
...
- When outside global namespace:
	...
	6. When importing multiple symbols from the same namespace, use a single import statement with a comma-separated list and each item on a new line. A single symbol stays on one line: `use Troy\Server\API;` Do not wrap a lone name in `{ }`.
	7. Import only first-order `Troy\Server` symbols: primary branches (`Admin`, `API`, `Endpoints`, `Integrations`, `Packages`, `Plugins`, `Settings`, `Stats`, `Themes`) and first-order classes (`Admin_Menu`, `Admin_Scripts`, `Cron`, `File_Utils`, `Markdown`, `Plugin_Table`, `Template`, `Zip_Extractor`). Do not import nested classes. Write those at the call site (`Plugins\Files::`, `Settings\Data::`, `Admin\Notice\Persistent::`, `Endpoints\Base_Endpoint`, `Integrations\Plugins\Store::`).
	...
	9. Same-namespace siblings and nested children of the current namespace stay unqualified (`new Data` and `CPT\Store::` in `Troy\Server\Plugins`). Do not import a branch that shadows a sibling of the current namespace (`Endpoints\Router` must not import `Plugins` or `Packages`).
